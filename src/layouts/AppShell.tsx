...
import { Bell, Lock, Menu, Sparkles, X } from 'lucide-react'

import { NotificationsPanel } from '@/components/NotificationsPanel'
import { Wordmark } from '@/components/Brand/Wordmark'
import { notifications } from '@/data/notifications'
import { macn, ticker, persona } from '@/data/macn'
                ...
                >
                  {({ isActive }) => (
                    <>
                      ...
                      {entry.badge?.variant === 'live' && (
                        <span className="flex items-center gap-1 rounded-full bg-signal-positive/15 px-1.5 py-0.5 font-mono text-[8.5px] font-semibold uppercase tracking-[0.06em] text-signal-positive">
                          <span className="h-1 w-1 animate-pulse-soft rounded-full bg-signal-positive" />
                          {entry.badge.text}
                        </span>
                      )}
                    </>
  ...
  return (
    <div className="px-4 pb-3 pt-3">
      <div className="relative overflow-hidden rounded-xl border border-white/10 bg-white/[0.04] px-3.5 py-3">
        <p className="font-mono text-[9px] font-medium uppercase tracking-[0.16em] text-steel-400">Auto-sanitized today</p>
        <div className="mt-1.5 flex items-baseline gap-1.5">
          <span className="font-display text-[20px] font-semibold tabular tracking-tight-bank text-white">{macn.autoSanitizedPct}%</span>
          <span className="font-mono text-[10px] text-azure-glow">{macn.reportsIngestedToday} ingested</span>
        </div>
        <div className="mt-2 flex items-center gap-3 border-t border-white/10 pt-2.5 text-[10px]">
          <span className="flex items-center gap-1 font-mono text-steel-300"><Lock className="h-3 w-3 text-steel-400" />{macn.vaultRestricted.toLocaleString()} vaulted</span>
          <span className="flex items-center gap-1 font-mono text-steel-300"><span className="h-1.5 w-1.5 rounded-full bg-signal-warning" />{macn.pendingReview} to review</span>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="border-t border-white/10 px-3 py-3">
      <button type="button" className="flex w-full items-center gap-2.5 rounded-md p-1.5 text-left transition-colors hover:bg-white/[0.05]">
        <div className="grid h-8 w-8 place-items-center rounded-full bg-azure text-[11px] font-semibold tabular text-white">{persona.initials}</div>
        <div className="min-w-0 flex-1">
          <p className="truncate text-[13px] font-medium text-white">{persona.name}</p>
          <p className="truncate font-mono text-[9px] uppercase tracking-[0.06em] text-steel-400">{persona.role}</p>
        </div>
      </button>
    </div>
  )
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center gap-4 border-b border-hairline bg-canvas/80 px-4 backdrop-blur-xl lg:px-8">
      ...
      <div className="min-w-0 shrink-0 lg:w-60">
        ...
        <p className="truncate font-display text-[16px] font-semibold tracking-tight-bank text-ink">{route?.label}</p>
      </div>
      ...
      <div className="hidden items-center gap-3 overflow-hidden lg:flex lg:flex-1">
        <span className="flex shrink-0 items-center gap-1.5 rounded-full border border-hairline bg-card px-2 py-1">
          ...
          <span className="font-mono text-[9px] font-semibold uppercase tracking-[0.1em] text-ink-subtle">Salesforce sync · live</span>
        </span>
        <div className="ticker-mask relative min-w-0 flex-1 overflow-hidden">
          <div className="flex w-max items-center gap-8 whitespace-nowrap will-change-transform animate-[ticker_42s_linear_infinite]">
      ...
      <button type="button" onClick={() => navigate('/copilot')} className="hidden shrink-0 items-center gap-2 rounded-full border border-azure/30 bg-azure-soft px-3 py-1.5 transition-colors hover:bg-azure-100 sm:flex">
        <Sparkles className="h-3.5 w-3.5 text-azure" />
        <span className="font-mono text-[10px] font-medium uppercase tracking-[0.08em] text-azure-deep">Ask Lodestar</span>
      </button>
    </header>
  )
}
